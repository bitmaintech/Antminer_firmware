...
int deal_cmd_connect_auth()
{
    send_connect_auth("10");
    rev_data_struct();
    if(strcmp(package.pkg_cmd,"f5")!=0)
    {
        return 0;
    }
    clear_recv(package);
...
int recv_confs()
{
    ...
    {
        rev_data_struct();
        if(strcmp(package.pkg_cmd,"09")!=0)
        {
            clear_recv(package);
            continue;
        }
        confs_json_format(package.pkg_nonce,server_command);
        confs_sum --;
        clear_recv(package);
        if(!confs_sum && !confs_full())
        {
            return 0;
        }
    }
    DEBUG_printf("confs_full:%d\n",confs_full());
    return 1;
}
...
int  rev_data_struct()
{
    ...
    sum=nonce_len[0]*256+nonce_len[1];//256 scale       // Calculate the length of the body
...
void send_connect_auth(char *cmd)
{
    ...
    authbody_size=strlen(gen_cuniqid(deal_package_cmd.pkg_nonce))+strlen(get_mac(mac_data,ETH))+strlen(ip_data)+strlen(userid)+strlen(dl_conf)+strlen(type);
...
//            strcpy(s4_dataformat_dir,dataformat_conf_dir[i]);
    ...
    statebody_size=strlen(data_format(order,buf_data,api_len));//s4_dataformat_dir
...
//    }else
    if(strcmp(pkg_struct.pkg_cmd,"06") == 0)
    {
        #ifdef OPENWRT
