...
void *pthr_deal_cmd()
{
    ...
    while(p_n_over)
    {
        ...
        else if(++time_count >= 660)//stop if no command after 11 mins
        {
...
int main(int argc, char *argv[])
{
    ...
    while(p_n_over)
    {
        if((Q_cmd.rear+1)%MAXQSIZE!=Q_cmd.front && (Q_respond.rear+1)%MAXQSIZE!=Q_respond.front)
        {
            ...
            else if(strcmp(package.pkg_cmd,"01") == 0)
            {
                DEBUG_printf("receive 01\n");
                if(!deal_cmd_connect_auth() || !recv_confs())
                {
                    raise(SIGINT);//exit(0);
                }
            }
