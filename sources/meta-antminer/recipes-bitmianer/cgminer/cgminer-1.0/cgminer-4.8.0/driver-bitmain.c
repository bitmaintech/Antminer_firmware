...
int opt_bitmain_fan_pwm = BITMAIN_DEFAULT_FAN_PWM;
...
static int bitmain_initialize(struct cgpu_info *bitmain)
{
	...
	if(statusok) {
		...
		applog(LOG_ERR, "2  opt_bitmain_fan_ctrl %d, opt_bitmain_fan_pwm %d",opt_bitmain_fan_ctrl,opt_bitmain_fan_ctrl);
		if(opt_bitmain_fan_ctrl){
			fan_eft = 1;
			if(opt_bitmain_fan_pwm){
				if(opt_bitmain_fan_pwm < 0 || opt_bitmain_fan_pwm > 100){
					applog(LOG_ERR,"set fan pwm error,ranges from 0~100");
					fan_pwm_data = BITMAIN_DEFAULT_FAN_MAX_PWM;
				}else 
					fan_pwm_data = opt_bitmain_fan_pwm	;						
			}
		...
		sendlen = bitmain_set_txconfig((struct bitmain_txconfig_token *)sendbuf, 1, fan_eft, 1, 1, 0, 0, 1, hwerror_eft, beeper_ctrl, tempover_ctrl,home_mode,use_vil,
				info->chain_num, info->asic_num, fan_pwm_data, info->timeout,
