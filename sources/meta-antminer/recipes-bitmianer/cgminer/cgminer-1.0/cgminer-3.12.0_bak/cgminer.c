...
#endif
	OPT_WITH_ARG("--work-proxy",
		     set_work_proxy, NULL, NULL,
		     "Enable work proxy, default: 127.0.0.1:9876"),
...
int work_proxy(struct pool * pool)
{
	if(opt_work_proxy == NULL)
		return 0;

	if(work_proxy_socket == -1) {
		work_proxy_socket = tcp_open(work_proxy_ip, work_proxy_port);
	}
}
