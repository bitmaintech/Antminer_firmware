...
bool get_gpio(FILE *pFile)
{
	...
	if((pRreco = fopen("/sys/class/gpio/gpio2/value", "r")) == NULL)
	{
	...
	while(1)
	{
		if(get_gpio(pRreco) == false)
		{
			...
			while(1)
			{
				...
				//3S
				if((1000000 * ( end.tv_sec - start.tv_sec ) + end.tv_usec - start.tv_usec) > (1000000 * 0.8))
				{
					printf("recovery button off over 1S, wait push on\n");
